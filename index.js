	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				error: {
					stick: '❌ Falha, ocorreu um erro ao converter a imagem em um adesivo, verifique e tente novamente ❌',
					Iv: '❌ Link invalido ❌'
				},
				only: {
					group: '❌ Este comando so pode ser usado em grupos! ❌',
					ownerG: '❌ Este comando so pode ser usado pelo grupo proprietorio!  ❌',
					ownerB: '❌ Este comando so pode ser usado pelo proprietorio do bot! ❌',
					admin: '❌ Este comando so pode ser usado por administradores de grupo! ❌',
					Badmin: '❌ Este comando so pode ser usado quando o bot se torna administrador! ❌'
				}
			}
			...
			switch(command) {
				...
				case 'ep':
					if (args.length < 1) {
						...
						return reply('O texto tambem, tio')
					}
					reply(mess.wait)
              ...
              reply(hasil)
                ...
                case 'leave':
                    ...
                    break
				case 'toimg':
					if (!isQuotedSticker) return reply('❌ responda stickernya hum ❌')
					reply(mess.wait)
